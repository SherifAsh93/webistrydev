export type Lang = "en" | "ar";

export const translations = {
  ar: {
    dir: "rtl" as const,
    lang: "ar",
    nav: {
      portfolio: "شغلي",
      services: "خدماتي",
      pricing: "الأسعار",
      howItWorks: "طريقة شغلي",
      hire: "قولّي فكرتك ←",
    },
    desktopCta: {
      label: "قولّي فكرتك ←",
    },
    hero: {
      badge: "تطبيقات موبايل، لوحات تحكم لحظية، وأنظمة إدارة",
      title1: "تطبيقات موبايل وأنظمة إدارة",
      title2: "مخصوصة لبيزنسك.",
      desc: "تطبيقات مبنية أول حاجة للموبايل، ولوحات تحكم لحظية تدير شغلك بالكامل — سريعة، وموثوقة، ومصممة خصيصًا ليك.",
      cta1: "احصل على عرض سعر مجاني",
      cta2: "شوف شغلي",
      stats: [
        { number: "9+", label: "فكرة اتبنت" },
        { number: "3+", label: "سنين خبرة" },
        { number: "⚡", label: "سرعة خرافية" },
        { number: "100%", label: "في الموعد دايمًا" },
      ],
      scroll: "اسحب للأسفل",
    },
    trustSignals: {
      items: ["100% كود مخصوص بالكامل", "9+ مشروع اتسلم فعلًا", "دعم 24/7"],
    },
    whyChooseMe: {
      sectionLabel: "ليه تختارني",
      title1: "ليه تختار",
      title2: "Webistrydev؟",
      desc: "كل مشروع بيتعمل بنفس الاهتمام — كود يعيش طويل، ودعم مايختفيش بعد التسليم.",
      items: [
        { title: "كود عالي الجودة", description: "كود نضيف وسهل التطوير ومبني بأحدث الطرق — من غير حلول سريعة أو ديون تقنية." },
        { title: "دعم عربي كامل (RTL)", description: "دعم كامل للعربي (من اليمين لليسار) والإنجليزي من أول يوم — مش إضافة لاحقة." },
        { title: "سنة صيانة مجانية", description: "إصلاح الأعطال والتحديثات البسيطة عليا لمدة سنة كاملة بعد الإطلاق، من غير أي تكلفة إضافية." },
      ],
    },
    portfolio: {
      sectionLabel: "شغلي",
      title1: "",
      title2: "مشاريعي",
      desc: "شغل حقيقي، عملاء حقيقيين.",
      liveSite: "شوف الموقع",
      buildLike: "عايز مشروع زيه ←",
      viewLive: "زور الموقع",
      live: "شغّال",
      flagshipBadge: "أحدث شغل",
      groups: {
        management: { label: "أنظمة الإدارة", desc: "لوحات تحكم لحظية وأنظمة إدارة بتدير البيزنس بالكامل." },
        ecommerce: { label: "تجارة إلكترونية", desc: "متاجر أونلاين متكاملة — من التصفح للدفع للتوصيل." },
        mobile: { label: "حلول ويب أول حاجة للموبايل", desc: "تطبيقات مبنية أول حاجة للموبايل — سريعة وحاسة إنها آب أصلي." },
      },
      caseLabels: { problem: "المشكلة", built: "الحل", outcome: "النتيجة" },
      bizApps: {
        badge: "برامج البيزنس",
        title1: "دير شغلك كله",
        title2: "من موبايلك",
        desc: "عيادة أو متجر أو أي بيزنس — بعمل برنامج يخلي شغلك اونلاين. المواعيد والسجلات والفواتير في إيدك في أي وقت من أي جهاز. مفيش برنامج تنزّله — افتح المتصفح وابدأ.",
        feats: [
          "شغّال على أي موبايل وحاسس إنه آب أصلي",
          "مواعيد وسجلات وفواتير لحظة بلحظة",
          "تتحكم فيه من أي حتة",
          "يناسب أي نوع بيزنس",
        ],
        builtFor: "اشتغلنا مع العملاء دول",
        cta: "ابني لبيزنسي",
      },
      ctaCard: {
        title: "فكرتك الجاية؟",
        desc: "عندك فكرة؟ قولهالنا وهنحولها لمنتج شغال.",
        btn: "احكيلي دلوقتي",
      },
    },
    projectModal: {
      referenceLabel: "عايز مشروع زي",
      title: "احكيلي عايز إيه بالظبط",
      subtitle: "شايف مشروع عجبك؟ قولّي عايز إيه وأنا هبنيهولك — هرد عليك خلال 24 ساعة.",
      close: "إغلاق",
    },
    services: {
      sectionLabel: "خدماتي",
      title: "إيه اللي بعمله",
      desc: "من موقع بسيط لبرنامج متكامل — كل حاجة ببنيها بسرعة وتخدم أهدافك.",
      quote: "احكيلي عن فكرتك ←",
      items: [
        { title: "متاجر اونلاين", description: "متجر كامل فيه منتجات وعربة ودفع آمن وأوردرات ولوحة تحكم تدير منها كل حاجة." },
        { title: "مواقع البراند", description: "مواقع فخمة تعرض مجموعاتك وتحول الزوار لعملاء." },
        { title: "برامج الإدارة", description: "عيادة أو مطعم أو صالون — بعمل برنامج يدير مواعيدك وسجلاتك وفواتيرك اونلاين." },
        { title: "برنامج خاص بيك", description: "عندك فكرة مش موجودة جاهزة؟ هبنيهالك من الصفر بالظبط اللي محتاجه." },
        { title: "مواقع الشركات", description: "موقع احترافي يبني ثقة العملاء ويجيب فرص جديدة." },
        { title: "صفحة بيع وتسويق", description: "الصفحة اللي عملاؤك الجدد بيوصلوا ليها بعد الإعلان — مصممة تحوّل كل زيارة لعملية بيع." },
      ],
    },
    pricing: {
      sectionLabel: "الأسعار",
      title1: "أسعار واضحة",
      title2: "من غير مفاجآت",
      desc: "الأسعار بتبدأ من النطاقات دي وبتتغير حسب الشغل. الدفع 50% أول ما نبدأ و50% عند التسليم.",
      popular: "الأكثر طلبًا",
      egpLabel: "ج.م.",
      usdLabel: "دولار",
      startingBadge: "يبدأ من 499$",
      startingNote: "أسعار خاصة متاحة للمشاريع المعقدة.",
      footerNote: "الأسعار نقطة بداية وبتتغير حسب الشغل. مش عارف أي باقة تناسبك؟",
      footerLink: "احكيلي وهشوفلك ←",
      tiers: [
        {
          name: "الأساسية",
          description: "ابدأ بيها — مناسبة للأفراد والمشاريع الصغيرة اللي عايزة حضور اونلاين سريع.",
          timeline: "1 – 2 أسبوع",
          features: ["1–3 صفحات", "شغّالة على كل الموبايلات", "فورم للتواصل", "بتظهر في جوجل", "استضافة سريعة وآمنة", "جولتين تعديل"],
          cta: "احكيلي فكرتك ←",
        },
        {
          name: "الأعمال",
          description: "للشغلانات اللي محتاجة موقع احترافي كامل تقدر تديره بنفسها.",
          timeline: "3 – 4 أسابيع",
          features: ["5–10 صفحات", "تصميم مخصوص ليك", "لوحة تحكم للمحتوى", "بتظهر في جوجل", "إحصائيات الزوار", "3 جولات تعديل", "30 يوم دعم بعد الإطلاق"],
          cta: "احكيلي فكرتك ←",
        },
        {
          name: "المتجر الإلكتروني",
          description: "ابيع اونلاين بواجهة احترافية وإدارة كاملة للمنتجات والطلبات.",
          timeline: "5 – 6 أسابيع",
          features: ["كتالوج منتجات كامل", "عربة ودفع آمن", "إدارة الأوردرات", "لوحة تحكم كاملة", "تتبع المخزون", "شغّال على كل الموبايلات", "تعديلات غير محدودة", "60 يوم دعم بعد الإطلاق"],
          cta: "احكيلي فكرتك ←",
        },
        {
          name: "برنامج خاص",
          description: "فكرة كبيرة أو بيزنس معقد؟ هبنيلك بالظبط اللي محتاجه.",
          timeline: "6+ أسابيع",
          features: ["بنية قوية تتحمل النمو", "دخول لكل مستخدم بصلاحيات", "بيتحدث لحظة بلحظة", "بياناتك محفوظة بأمان", "بيتوصل بأي خدمة تانية", "متحتاجش تقلق من أعطال", "تعديلات غير محدودة", "90 يوم دعم بعد الإطلاق"],
          cta: "احكيلي فكرتك ←",
        },
      ],
    },
    hireCTA: {
      badge: "✦ يلا نبدأ",
      headline: "فكرتك تستاهل.",
      sub: "احكيلي اللي في بالك — وهحوّله لمنتج حقيقي وشغال.",
      cta: "ابدأ مشروعك ←",
    },
    howItWorks: {
      sectionLabel: "الطريقة",
      title1: "إزاي بنشتغل",
      title2: "مع بعض",
      desc: "خطوات واضحة ومحددة — عشان تعرف دايمًا الشغل وصل لفين.",
      steps: [
        { title: "بنتكلم", description: "قولّي عن فكرتك وميزانيتك — ونتفق على التفاصيل وخطة واضحة." },
        { title: "بنصمم ونوافق", description: "بعمل ليك تصميم كامل — بتراجعه وتدي رأيك وتوافق عليه قبل ما نبني." },
        { title: "بنبني ونختبر", description: "البناء يبدأ وبختبره على كل الأجهزة — وبخليك على اطلاع طول الوقت." },
        { title: "بنطلق ونكمّل", description: "موقعك يطلع للدنيا وأنا بتكفل بكل حاجة بعد الإطلاق." },
      ],
    },
    startProject: {
      sectionLabel: "قولّي وأنا أتكفل",
      title1: "فكرتك في بالك؟",
      title2: "قولهالي دلوقتي.",
      desc: "مش محتاج تعرف أي حاجة تقنية — بس احكيلي عايز إيه وأنا اللي هتكفل بالباقي خالص.",
      bookConsultation: "احجز مكالمة مجانية 15 دقيقة",
      trustBadges: ["⚡ هرد عليك في نفس اليوم", "💬 استشارة مجانية 100%", "🔐 بيننا بس"],
      typePickerLabel: "إيه اللي عايزه؟",
      projectTypes: [
        { key: "store", label: "🛍️ متجر اونلاين" },
        { key: "brand", label: "🌐 موقع براند" },
        { key: "app", label: "📱 برنامج خاص" },
        { key: "landing", label: "📣 صفحة تسويق" },
        { key: "clinic", label: "🏥 نظام عيادة" },
        { key: "other", label: "🎨 حاجة تانية" },
      ],
      typePlaceholders: {
        store: "احكيلي عن متجرك — بتبيع إيه؟ عندك منتجات دلوقتي؟ بتشتغل بالتوصيل؟ عايز دفع اونلاين؟ قولّي كل اللي في بالك...",
        brand: "احكيلي عن براندك — إيه اللي بتبيعه أو بتقدّمه؟ مين عملاؤك؟ عايز الموقع يعمل إيه بالظبط؟ لو عندك مثال عجبك قوله...",
        app: "احكيلي عن الفكرة — البرنامج ده هيخدم مين؟ هيعمل إيه بالظبط؟ عندك لوجو أو تصور أولي؟ ماشي، احكيلي كل اللي في بالك...",
        landing: "احكيلي عن منتجك أو خدمتك — بتبيع إيه؟ بتستهدف مين؟ الإعلانات بتروح على فين دلوقتي؟ عايز الصفحة تعمل إيه بالظبط؟...",
        clinic: "احكيلي عن عيادتك — تخصصها إيه؟ عايز نظام مواعيد؟ ملفات مرضى؟ فواتير؟ عندك كام دكتور بتشتغل معاهم؟...",
        other: "احكيلي أي حاجة في بالك — مش لازم تعرف التفاصيل التقنية. بس ابدأ واوصفلي إيه اللي عايزه وأنا هفهم وهقترحلك الأفضل...",
      },
      voiceTitle: "اضغط وابدأ احكي",
      voiceHint: "سجّل رسالة صوتية وقولّي كل اللي في بالك",
      voiceRequesting: "جاري تفعيل المايك...",
      voiceRecording: "بيتسجل",
      voiceStop: "خلاص، وقّف",
      voiceRecorded: "اتسجّلت",
      voicePlay: "استمع",
      voicePause: "وقّف",
      voiceRerecord: "سجّل تاني",
      textLabel: "احكيلي عن فكرتك",
      voiceAddNote: "إضافة رسالة صوتية (اختياري)",
      voiceRemoveNote: "إزالة",
      textPlaceholder: "احكيلي فكرتك — عايز تبني إيه؟ للمين؟ هيعمل إيه؟ مش لازم تعرف حاجة، بس اوصفلي وأنا هتكفل بالباقي...",
      nameLabel: "اسمك",
      namePlaceholder: "اسمك الكامل",
      phoneLabel: "واتساب / موبايل",
      phonePlaceholder: "+20 100 000 0000",
      contactNote: "* بس اسمك ورقم تليفونك وأنا هوصلك.",
      submit: "ابعت فكرتي ←",
      sending: "بيتبعت...",
      whatsappDirect: "أو كلمني مباشرة على واتساب",
      nextStepsTitle: "إيه اللي بيحصل بعد ما تبعت؟",
      nextSteps: [
        "هواصلك على الواتساب في نفس اليوم",
        "نتكلم عن الفكرة وتوقعاتك مجانًا",
        "هديك سعر وخطة واضحة — بدون أي التزام",
      ],
      successTitle: "وصلتني! 🎉",
      successMessage: "شكرًا {name}! استلمت رسالتك وهتواصل معاك خلال 24 ساعة.",
      successNote: "هوصلك على الواتساب أو المكالمة.",
      successAgain: "ابعت تاني",
      chatSaveTitle: "🔗 احتفظ بالرابط ده كويس!",
      chatSaveDesc: "ده رابط محادثتنا الخاصة مع بعض — أي وقت عندك سؤال أو عايز تضيف تفاصيل، ابعتهالي من هنا وهتشوف ردودي في أي وقت من غير ما تحتاج أي حاجة تانية.",
      chatCopy: "نسخ الرابط",
      chatCopied: "اتنسخ ✓",
      chatOpen: "افتح المحادثة ←",
    },
    footer: {
      desc: "بنبني مواقع وبرامج سريعة وعصرية للشركات ورواد الأعمال في كل حتة.",
      cta: "ابدأ مشروعك ←",
      navTitle: "روابط",
      navLinks: [
        { label: "شغلي", href: "#portfolio" },
        { label: "خدماتي", href: "#services" },
        { label: "الأسعار", href: "#pricing" },
        { label: "طريقة شغلي", href: "#how-it-works" },
        { label: "قولّي فكرتك", href: "#start-project" },
      ],
      contactTitle: "تواصل",
      whatsapp: "واتساب (الأسرع)",
      rights: `© ${new Date().getFullYear()} Webistrydev. كل الحقوق محفوظة.`,
      built: "صُنع بحب لعملاء حول العالم ❤️",
    },
    bottomNav: {
      home: "الرئيسية",
      work: "شغلي",
      pricing: "الأسعار",
      contact: "تواصل",
    },
    projectDescs: {
      "ahmed-elakad": "موقع فاخر لأشهر مصمم أزياء — بيعرض مجموعاته ويخلي عملاؤه يحجزوا مباشرة.",
      "furniture-studio": "متجر أثاث راقي — اتفرج على القطع، اختار اللي يعجبك، واطلب اونلاين.",
      "zahrtelkhlig": "متجر حجاب متكامل — منتجات ومقاسات وأوردرات وتوصيل، كل حاجة من موبايلك.",
      "batrawy-clinic": "نظام إدارة عيادة — مواعيد ومرضى وفواتير، كل ده اونلاين من أي جهاز في أي وقت.",
      "ameer-dental": "عيادة أسنان اونلاين — حجز مواعيد وملفات مرضى وتتبع العلاجات بكل سهولة.",
      "elghaly-vr": "برنامج ذكي بيلتقط الألوان من الكاميرا ويريك إزاي هتبان في أوضتك قبل ما تصبغها.",
      "qoya-furniture": "موقع براند فاخر لشوروم أثاث راقي — تصميم تحريري كامل الشاشة، 90+ صورة احترافية، ونظام استفسار مع لوكيتور الفروع.",
      "olympia-beach-club": "تطبيق موبايل لنادي شاطئ راقي — حجز مرافق، فعاليات، اشتراكات، وتجربة شبه أبليكيشن على الموبايل.",
      "mr-mohammed": "موقع أستاذ لغة عربية وتربية إسلامية — بيسجّل الطلاب في الدورات أونلاين، بيدفعوا عبر فودافون كاش، ولوحة تحكم سرية للإدارة الكاملة.",
      "sunset-management": "لوحة تحكم إدارية لكمبوند خاص — بحث لحظي ولوحة تحكم كاملة لتسيير الشغل اليومي.",
      "webistrydev": "الموقع ده — منصة لجذب العملاء فيها فورم استفسار بالصوت أو الكتابة، لوحة تحكم إدارية، شات مباشر مع العميل، ودعم كامل للعربي والإنجليزي.",
    } as Record<string, string>,
    caseStudies: {
      "mr-mohammed": {
        problem: "أستاذ لغة عربية وتربية إسلامية معندوش وسيلة يبيع أو يدير بيها دوراته اونلاين غير المكالمات والواتساب — مفيش تسجيل ذاتي، ولا دفع اتوماتيك، ولا سجل للطلاب.",
        built: "منصة كورسات شغّالة من الموبايل أول حاجة — الطالب يتصفح ويسجل بنفسه، يدفع بفودافون كاش، ولوحة تحكم سرية تدير الكورسات والتسجيلات والمواد والنتائج.",
        outcome: "التسجيل والدفع بقوا من غير مكالمة — الأستاذ بيدير شغله كله من لوحة تحكم واحدة بدل محادثات متفرقة وورقة وقلم.",
      },
      "batrawy-clinic": {
        problem: "العيادة كانت شغالة بسجلات ورقية ومواعيد بالتليفون — بطيء للموظفين وسهل يحصل حجز مزدوج، ومفيش رؤية واحدة لتاريخ المريض أو حسابه.",
        built: "نظام إدارة عيادة كامل — سجلات مرضى، حجز مواعيد، فواتير، ولوحة تحكم للدكتور، شغال من أي موبايل زي أبليكيشن.",
        outcome: "الموظفين بيحجزوا ويديروا المواعيد من الموبايل في ثواني؛ تاريخ المريض والفواتير بقوا في نظام واحد قابل للبحث بدل دفتر ورقي.",
      },
      "sunset-management": {
        problem: "شغل الكمبوند اليومي معندوش نظام مركزي — الموظفين محتاجين بحث سريع في بيانات السكان والطلبات ومكانش عندهم.",
        built: "لوحة تحكم إدارية ببحث لحظي ولوحة تحكم كاملة لتسيير الشغل اليومي.",
        outcome: "الموظفين بيلاقوا أي بيانات في ثواني بدل ما يدوروا في شيتات إكسل — لوحة واحدة قامت مقام أدوات متفرقة كتير.",
      },
      "ameer-dental": {
        problem: "الحجز ومتابعة المرضى كانوا معتمدين بالكامل على المكالمات والملاحظات اليدوية.",
        built: "حجز مواعيد ذكي، إدارة مرضى، وتتبع علاج، كله من الموبايل أول حاجة.",
        outcome: "المرضى بيحجزوا من غير ما يتصلوا؛ العيادة بتتابع تاريخ علاج كل مريض في مكان واحد بدل ملفات ورقية متفرقة.",
      },
      "zahrtelkhlig": {
        problem: "براند حجاب كان بيبيع أساسًا عن طريق الإنستجرام والتوصية الشخصية، من غير وسيلة يستقبل بيها أوردرات أو يتابع مخزون بشكل كبير.",
        built: "متجر إلكتروني كامل — كتالوج، اختيار مقاس، مفضلة، عربة، دفع، تتبع أوردرات، نظام كاشير (POS)، ولوحة تحكم، كله من الموبايل أول حاجة.",
        outcome: "العملاء بيشتروا مباشرة من الموقع في أي وقت بدل رسايل الإنستجرام المتبادلة؛ المخزون والأوردرات متابعة من لوحة تحكم واحدة مربوطة بالكاشير.",
      },
      "ahmed-elakad": {
        problem: "مصمم أزياء فاخر محتاج حضور رقمي يواكب مستوى البراند، ووسيلة يستقبل بيها طلبات مواعيد غير رسايل الإنستجرام.",
        built: "جاليري مخصوص، إدارة كولكشنز، ونظام حجز مواعيد.",
        outcome: "واجهة دائمة وعلى مستوى البراند يقدر العميل يتصفحها في أي وقت، وطلبات الحجز بقت توصل من الموقع مش بس عن طريق الرسايل.",
      },
      "furniture-studio": {
        problem: "متجر أثاث معماري كان محتاج يبيع اونلاين فعليًا — بإدارة أوردرات ومخزون حقيقية، مش مجرد صفحة كتالوج.",
        built: "متجر إلكتروني كامل بإدارة منتجات، عربة، أوردرات، ولوحة تحكم.",
        outcome: "الأوردرات والمخزون متابعين من لوحة تحكم واحدة بدل التوفيق اليدوي بين قنوات متفرقة.",
      },
      "qoya-furniture": {
        problem: "شوروم أثاث فاخر محتاج حضور على الويب بنفس تأثير دخول المحل، وطريقة سهلة يلاقي بيها الفرع الأقرب.",
        built: "تصميم تحريري كامل الشاشة بسلايدر تلقائي، أكتر من 90 صورة منتج، نظام استفسار، ولوكيتور لفرعين.",
        outcome: "الموقع بقى شبه شوروم تاني ممكن تتصفحه في أي وقت، وبيبعت استفسارات للبيزنس مباشرة بدل الاعتماد على اللي بيمر بالمحل.",
      },
      "elghaly-vr": {
        problem: "العملاء كانوا محتاجين يتخيلوا لون الدهان هيبان إزاي في مكانهم، أو يعتمدوا على عينات فيزيائية.",
        built: "برنامج AR بيلتقط ألوان حقيقية من كاميرا الموبايل ويدهن أوضة افتراضية لحظيًا.",
        outcome: "العميل بيجرب اللون في أوضته الحقيقية قبل ما يشتري — بيقلل الحيرة اللي بتؤدي لمرتجعات.",
      },
      "olympia-beach-club": {
        problem: "النادي معندوش تجربة حجز رقمية — استخدام المرافق والاشتراكات كان بيتم يدويًا، ومفيش حاجة تبين شكل النظام الحديث.",
        built: "حجز مرافق، تسجيل فعاليات، وباقات اشتراك، ثنائي اللغة عربي/إنجليزي، اتبنى كنموذج عرض.",
        outcome: "تجربة حجز جاهزة للإطلاق يقدر النادي يقيّمها قبل ما يلتزم برقمنة العمليات.",
      },
    } as Record<string, { problem: string; built: string; outcome: string }>,
    categoryLabels: {
      fashion: "براند أزياء",
      ecommerce: "تجارة إلكترونية",
      clinic: "برنامج ويب",
      "web-app": "برنامج ويب",
      corporate: "موقع براند فاخر",
      landing: "صفحة ترويجية",
      self: "موقع الشركة",
    } as Record<string, string>,
  },
  en: {
    dir: "ltr" as const,
    lang: "en",
    nav: {
      portfolio: "Portfolio",
      services: "Services",
      pricing: "Pricing",
      howItWorks: "How It Works",
      hire: "Get Free Quote →",
    },
    desktopCta: {
      label: "Get a Free Quote →",
    },
    hero: {
      badge: "10+ Projects Delivered · Mobile-First · Fast Turnaround",
      title1: "More Clients. More Sales.",
      title2: "Run It From Your Phone.",
      desc: "Custom mobile-first apps for business owners — online stores, booking systems, and management dashboards built to turn visitors into paying customers.",
      cta1: "Get My Free Quote",
      cta2: "See My Work",
      stats: [
        { number: "10+", label: "Projects Built" },
        { number: "3+", label: "Years Experience" },
        { number: "⚡", label: "Lightning Fast" },
        { number: "100%", label: "On-Time Delivery" },
      ],
      scroll: "Scroll",
    },
    trustSignals: {
      items: ["100% Custom Code", "10+ Projects Delivered", "24/7 Support"],
    },
    whyChooseMe: {
      sectionLabel: "Why Choose Me",
      title1: "Why Choose",
      title2: "Webistrydev?",
      desc: "Every project is built with the same care — code that lasts, and support that doesn't disappear after launch.",
      items: [
        { title: "High-Quality Code", description: "Clean, maintainable code built with modern best practices — no shortcuts, no technical debt." },
        { title: "RTL / Arabic Support", description: "Full bilingual Arabic (RTL) and English support built in from day one — not bolted on later." },
        { title: "1-Year Free Maintenance", description: "Bug fixes and minor updates are on me for a full year after launch, at no extra cost." },
      ],
    },
    portfolio: {
      sectionLabel: "Portfolio",
      title1: "",
      title2: "My Work",
      desc: "Real clients, real results.",
      liveSite: "Live Site",
      buildLike: "I Want Something Like This →",
      viewLive: "View Live Site",
      live: "Live",
      flagshipBadge: "Latest Build",
      groups: {
        management: { label: "Management Systems", desc: "Real-time dashboards and management systems that run the business." },
        ecommerce: { label: "E-Commerce", desc: "Full online stores — from browsing to checkout to fulfillment." },
        mobile: { label: "Mobile-First Web Solutions", desc: "Apps built mobile-first — fast, native-feeling, and always in your pocket." },
      },
      caseLabels: { problem: "The Problem", built: "What I Built", outcome: "The Outcome" },
      bizApps: {
        badge: "Business Apps",
        title1: "Run Your Entire Business",
        title2: "From Your Phone",
        desc: "Whether it's a clinic, a store, or any kind of business — I build apps that put your entire operation online. Manage appointments, records, and billing in real time from any device. No download needed. Just open the browser and you're in.",
        feats: [
          "Works on any phone — feels like a native app",
          "Real-time appointments, records & billing",
          "Full control from anywhere",
          "Adaptable to any business type",
        ],
        builtFor: "Built for These Clients",
        cta: "Build for My Business",
      },
      ctaCard: {
        title: "Your Idea Next?",
        desc: "Have an idea? Let's turn it into a fast, beautiful, fully-functional product together.",
        btn: "Tell Me Now",
      },
    },
    projectModal: {
      referenceLabel: "I want something like",
      title: "Tell Me What You Need",
      subtitle: "You saw something you liked — tell me what you want and I'll build it for you. I'll get back within 24 hours.",
      close: "Close",
    },
    services: {
      sectionLabel: "Services",
      title: "What I Build",
      desc: "From a simple app to a full product — everything is built fast and designed around your goals.",
      quote: "Tell me your idea →",
      items: [
        { title: "E-Commerce App", description: "Full store with products, cart, secure checkout, order tracking, and an admin panel to manage everything." },
        { title: "Brand App", description: "Elegant apps that showcase your collections and turn visitors into clients." },
        { title: "Business App", description: "Clinic, restaurant, salon, or any business — I build an app to manage your appointments, records, and billing all online." },
        { title: "Anything Custom", description: "Have an idea that doesn't exist off the shelf? I'll build it from scratch — exactly the way you need it." },
        { title: "Corporate App", description: "Professional apps that build trust and bring in new clients." },
        { title: "Promo App", description: "One focused platform built to attract customers, collect leads, and grow your business." },
      ],
    },
    pricing: {
      sectionLabel: "Pricing",
      title1: "Transparent Pricing,",
      title2: "No Surprises",
      desc: "All prices start from the ranges below and vary by project scope. Payment is 50% upfront, 50% on delivery.",
      popular: "Most Popular",
      egpLabel: "EGP",
      usdLabel: "USD",
      startingBadge: "Starting at $499 USD",
      startingNote: "Custom quotes available for complex projects.",
      footerNote: "Prices are starting points and vary based on scope. Not sure which fits?",
      footerLink: "Tell me and I'll guide you →",
      tiers: [
        {
          name: "Basic App",
          description: "A fast, professional site that makes your business look legit online — live in 1–2 weeks.",
          timeline: "1 – 2 weeks",
          features: ["1–3 platform app", "Looks great on any device", "Contact form", "Shows up on Google", "Fast, secure hosting", "2 revision rounds"],
          cta: "Get a Custom Quote",
        },
        {
          name: "Business App",
          description: "A complete business app with your own admin panel — manage everything from your phone, no developer needed after launch.",
          timeline: "3 – 4 weeks",
          features: ["5–10 platform app", "Custom design just for you", "Admin panel to manage your content", "Shows up on Google", "Visitor stats & performance reports", "3 revision rounds", "30 days post-launch support"],
          cta: "Get a Custom Quote",
        },
        {
          name: "E-Commerce App",
          description: "Sell online with a real store — cart, checkout, and order tracking — without a marketplace's fees or rules.",
          timeline: "5 – 6 weeks",
          features: ["Full product catalog", "Cart & secure checkout", "Order management", "Admin dashboard", "Inventory tracking", "Looks perfect on any phone", "Unlimited revisions during build", "60 days post-launch support"],
          cta: "Book a Free Consultation",
        },
        {
          name: "Enterprise App",
          description: "Custom systems built around how your business actually runs — not squeezed into someone else's template.",
          timeline: "6+ weeks",
          features: ["Built to grow with you", "Login system with access levels", "Updates live without refreshing", "Stores all your data securely", "Connects to any service you need", "Tested to work without issues", "Unlimited revisions", "90 days post-launch support"],
          cta: "Get a Custom Quote",
        },
      ],
    },
    hireCTA: {
      badge: "✦ Let's Build",
      headline: "Your Idea Deserves to Be Real.",
      sub: "Tell me what you want to build — I'll take it from there.",
      cta: "Start Your Project →",
    },
    howItWorks: {
      sectionLabel: "Process",
      title1: "How We Work",
      title2: "Together",
      desc: "A clear, simple process — so you always know where things stand.",
      steps: [
        { title: "We Talk", description: "Tell me about your idea, goals, and budget. We agree on what to build, the timeline, and a clear plan." },
        { title: "Design & Approve", description: "I create a detailed mockup. You review, give feedback, and approve before anything gets built." },
        { title: "Build & Test", description: "I build everything and test it on all devices — keeping you updated the whole way." },
        { title: "Launch & Support", description: "Your product goes live. I handle everything and stay available after launch." },
      ],
    },
    startProject: {
      sectionLabel: "Tell Me, I'll Handle It",
      title1: "Have an idea?",
      title2: "Tell me now.",
      desc: "No technical knowledge needed — just describe what you want and I'll take it from there.",
      bookConsultation: "Book a Free 15-min Consultation",
      trustBadges: ["⚡ I reply same day", "💬 Free consultation", "🔐 Just between us"],
      typePickerLabel: "What are you looking for?",
      projectTypes: [
        { key: "store", label: "🛍️ Online Store" },
        { key: "brand", label: "🌐 Brand Website" },
        { key: "app", label: "📱 Custom App" },
        { key: "landing", label: "📣 Landing Page" },
        { key: "clinic", label: "🏥 Clinic System" },
        { key: "other", label: "🎨 Something else" },
      ],
      typePlaceholders: {
        store: "Tell me about your store — what are you selling? Do you have existing products? Do you need delivery? Online payments? Tell me everything...",
        brand: "Tell me about your brand — what do you sell or offer? Who are your customers? What do you want the site to do exactly? Share any examples you like...",
        app: "Tell me about your idea — who will use this app? What should it do exactly? Do you have a logo or initial concept? Just tell me what's on your mind...",
        landing: "Tell me about your product or service — what are you selling? Who's your audience? Where are your ads pointing right now? What should the page achieve?...",
        clinic: "Tell me about your clinic — what's the specialty? Do you need appointment booking? Patient records? Billing? How many doctors are involved?...",
        other: "Tell me anything — no need to know the technical details. Just describe what you want and I'll figure out the best approach and suggest a plan...",
      },
      voiceTitle: "Tap & Start Talking",
      voiceHint: "Record a voice note — tell me everything you have in mind",
      voiceRequesting: "Activating microphone...",
      voiceRecording: "Recording",
      voiceStop: "Done, stop",
      voiceRecorded: "Recorded",
      voicePlay: "Play back",
      voicePause: "Pause",
      voiceRerecord: "Re-record",
      textLabel: "Tell me about your idea",
      voiceAddNote: "Add a voice note (optional)",
      voiceRemoveNote: "Remove",
      textPlaceholder: "Tell me your idea — what do you want to build? Who's it for? What should it do? No technical knowledge needed, just describe it and I'll handle the rest...",
      nameLabel: "Your Name",
      namePlaceholder: "Full name",
      phoneLabel: "WhatsApp / Phone",
      phonePlaceholder: "+20 100 000 0000",
      contactNote: "* Just your name and phone number — I'll reach out to you.",
      submit: "Send My Idea →",
      sending: "Sending...",
      whatsappDirect: "Or message me directly on WhatsApp",
      nextStepsTitle: "What happens after you send?",
      nextSteps: [
        "I'll message you on WhatsApp the same day",
        "We'll talk about your idea and goals — for free",
        "You'll get a clear price and plan — no commitment",
      ],
      successTitle: "Got it! 🎉",
      successMessage: "Thanks {name}! I received your message and will get back to you within 24 hours.",
      successNote: "I'll reach you on WhatsApp or by phone.",
      successAgain: "Send another",
      chatSaveTitle: "🔗 Save this link!",
      chatSaveDesc: "This is our private conversation space — message me anytime with questions or extra details, and you'll see my replies here whenever you want, no account needed.",
      chatCopy: "Copy Link",
      chatCopied: "Copied ✓",
      chatOpen: "Open Chat →",
    },
    footer: {
      desc: "Global software solutions for modern businesses — fast, modern websites and apps for entrepreneurs worldwide.",
      cta: "Start a Project →",
      navTitle: "Navigation",
      navLinks: [
        { label: "Portfolio", href: "#portfolio" },
        { label: "Services", href: "#services" },
        { label: "Pricing", href: "#pricing" },
        { label: "How It Works", href: "#how-it-works" },
        { label: "Share Your Idea", href: "#start-project" },
      ],
      contactTitle: "Contact",
      whatsapp: "WhatsApp (fastest)",
      rights: `© ${new Date().getFullYear()} Webistrydev. All rights reserved.`,
      built: "Made with ❤️, serving clients worldwide",
    },
    bottomNav: {
      home: "Home",
      work: "Work",
      pricing: "Pricing",
      contact: "Contact",
    },
    projectDescs: {
      "ahmed-elakad": "Luxury website for a premier fashion designer — showcasing his collections and letting clients book appointments.",
      "furniture-studio": "Premium furniture store — browse beautiful pieces, choose your favorites, and order online.",
      "zahrtelkhlig": "Full hijab fashion store — products, sizes, orders, and delivery, all from your phone.",
      "batrawy-clinic": "Clinic management system — appointments, patient records, and billing, all online from any device.",
      "ameer-dental": "Dental clinic online — easy appointment booking, patient files, and treatment tracking.",
      "elghaly-vr": "Smart app that captures room colors through your camera and shows how they'll look before you paint.",
      "qoya-furniture": "Ultra-luxury brand website for a premium furniture showroom — fullscreen editorial design, 90+ professional photos, inquiry system, and dual-branch store locator.",
      "olympia-beach-club": "Mobile-first beach club app — facility booking, events, membership plans, and a native app feel on any phone.",
      "mr-mohammed": "Arabic & Islamic Studies teacher platform — students enroll in live online courses, pay via Vodafone Cash, and the teacher manages everything from a hidden admin panel.",
      "sunset-management": "Managerial dashboard platform for a private compound — real-time search and a full admin dashboard for day-to-day operations.",
      "webistrydev": "This site — a lead-generation platform with a voice-or-text inquiry form, admin dashboard, live client chat, and bilingual AR/EN support.",
    } as Record<string, string>,
    caseStudies: {
      "mr-mohammed": {
        problem: "An Arabic & Islamic Studies teacher had no way to sell or manage live online courses beyond phone calls and WhatsApp — no self-serve enrollment, no automated payment, no student records.",
        built: "A mobile-first course platform where students browse and enroll themselves, pay by Vodafone Cash, and a hidden admin panel manages courses, enrollments, materials, and results.",
        outcome: "Enrollment and payment now happen without a phone call — the teacher runs the whole operation from one admin panel instead of scattered chats and spreadsheets.",
      },
      "batrawy-clinic": {
        problem: "The clinic ran on paper records and phone-booked appointments — slow for staff, easy to double-book, no single view of a patient's history or billing.",
        built: "A full clinic management system: patient records, appointment scheduling, billing, and a doctor dashboard, usable from any phone like a native app.",
        outcome: "Staff book and manage appointments from a phone in seconds; patient history and billing live in one searchable system instead of a paper ledger.",
      },
      "sunset-management": {
        problem: "Day-to-day operations for a private compound had no central system — staff needed fast lookups across residents and requests, and had none.",
        built: "A managerial dashboard with real-time search and a full admin panel for daily operations.",
        outcome: "Staff pull up any record in seconds instead of digging through spreadsheets — one dashboard replaced several disconnected tools.",
      },
      "ameer-dental": {
        problem: "Booking and patient tracking depended entirely on phone calls and manual notes.",
        built: "Smart appointment booking, patient management, and treatment tracking, mobile-first.",
        outcome: "Patients book without calling; the clinic tracks each patient's treatment history in one place instead of loose paper files.",
      },
      "zahrtelkhlig": {
        problem: "A hijab fashion brand sold mainly through Instagram DMs and word-of-mouth, with no way to take orders or track inventory at scale.",
        built: "Full e-commerce — catalog, size picker, wishlist, cart, checkout, order tracking, a POS system, and an admin panel, built mobile-first.",
        outcome: "Customers buy directly on the site any time of day instead of DM back-and-forth; stock and orders are tracked from one POS-linked admin panel.",
      },
      "ahmed-elakad": {
        problem: "A premium fashion designer needed a digital presence matching the brand's positioning, and a way to take appointment requests beyond Instagram DMs.",
        built: "A custom gallery, collection management, and an appointment booking system.",
        outcome: "A permanent, on-brand showcase clients can browse any time, with booking requests landing on the site instead of only in DMs.",
      },
      "furniture-studio": {
        problem: "An architectural furniture marketplace needed to actually sell online — with real order and inventory management, not just a catalog page.",
        built: "Full e-commerce with product management, cart, orders, and an admin dashboard.",
        outcome: "Orders and inventory are tracked in one dashboard instead of reconciled by hand across channels.",
      },
      "qoya-furniture": {
        problem: "A luxury furniture showroom needed a web presence with the same impact as walking into the store, plus an easy way to find the nearest branch.",
        built: "A fullscreen editorial design with an auto-cycling hero slider, 90+ product images, a contact inquiry system, and a two-branch store locator.",
        outcome: "The site functions as a second showroom that's browsable any time, sending inquiries straight to the business instead of relying on walk-ins.",
      },
      "elghaly-vr": {
        problem: "Customers had to imagine how a paint color would look in their space, or rely on physical swatches.",
        built: "An AR room visualizer that captures real-world colors from a phone camera and paints a virtual room in real time.",
        outcome: "Customers preview a color in their actual room before buying — cutting the guesswork that leads to returns.",
      },
      "olympia-beach-club": {
        problem: "The club had no digital booking experience — facility use and memberships were handled manually, with nothing to show what a modern system could look like.",
        built: "Facility booking, event registration, and membership plans, bilingual AR/EN, built as a pitch demo.",
        outcome: "A ready-to-launch booking experience the club can evaluate before committing to digitize operations.",
      },
    } as Record<string, { problem: string; built: string; outcome: string }>,
    categoryLabels: {} as Record<string, string>,
  },
};

export type Translations = typeof translations.en;
